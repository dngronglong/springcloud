...
$(function () {
    ...
    $("#sheet").on("click", ".sheet-cover,.sheet-name", function () {
        var num = parseInt($(this).parent().data("no"));
        // 是用户列表，但是还没有加载数据
        if (musicList[num].item.length === 0 && musicList[num].id < 4) {
...
function searchBox() {
    var tmpHtml = '<form onSubmit="return searchSubmit()"><div id="search-area">' +
        ...
        '       <label><input type="radio" name="source" value="kugou" checked=""> 酷狗</label>' +
...
function loadList(list) {
            ...
            if (parmeter == 'top')
                for (var i = 0; i < data.top.length; i++) {
                    musicList[list].item.push({
                        name: data.top[i].song.musicName,
                        album: data.top[i].song.album,
                        mid: data.top[i].song.mid,
                        artist: data.top[i].song.listSinger[0].name
                    })
                } else
                for (var i = 0; i < data.random.length; i++) {
                    musicList[list].item.push({
                        name: data.random[i].song.musicName,
                        album: data.random[i].song.album,
                        mid: data.random[i].song.mid,
                        artist: data.random[i].song.listSinger[0].name
                    })
                }
        }
    });
    ...
    if (mkPlayer.debug) {
        ...
        //console.log("加载的数据为"+musicList[list].item);
    }
...
function initList() {
    ...
    for (var i = 0; i < musicList.length; i++) {
        ...
        if (i == 1) {    // 正在播放列表
            ...
            var tmp_item = playerReaddata("top")
            if (tmp_item) {
            ...
            var tmp_item = playerReaddata("random")
            if (tmp_item) {
                musicList[4].item = tmp_item;
            } else {
                loadList(4);
            }
        }
